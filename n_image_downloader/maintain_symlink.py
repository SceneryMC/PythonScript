import json
import os
from secret import map_classifier_to_folder, sync_path, classifiers, artist_path
...
# map_classifier_to_folder: dict[str, str]
...
if __name__ == '__main__':
    ...
    add_new_classifiers()
    # root = r'F:\存储\其它\SYNC\TAG'
    # for base, folders, files in os.walk(root):
    #     if folders:
    #         continue
    #     for file in files:
    #         if file.isdigit() and int(file) < 1000000:
    #             os.remove(rf"{base}\{file}")
    #             os.symlink(rf"{local_path[file]}\{file}", rf"{base}\{file}")
    #             print(rf"RECONNECTED {local_path[file]}\{file}")
